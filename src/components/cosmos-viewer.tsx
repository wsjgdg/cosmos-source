  ...
  <svg viewBox="0 0 436 336" width="100%" style={{ maxWidth: 436 }}>
    <defs>
      <radialGradient id="gOcean" cx=".38" cy=".32" r=".85">
        <stop offset="0" stopColor="#2e86bd" />
        <stop offset="1" stopColor="#0f2f4a" />
      </radialGradient>
    ...
    <g stroke="#f5a623" strokeOpacity=".5" strokeWidth="1">
      ...
      <path d="M6 86 L10 74 L14 86" fill="none" />
      <path d="M6 236 L10 248 L14 236" fill="none" />
    </g>
    <path
      d="M112 56 C110 34, 122 30, 118 8 C132 24, 134 40, 132 56 Z"
      fill="url(#gPlume)"
    />
    ...
    <g stroke="#8fb8d4" strokeOpacity=".5" fill="none">
      <path d="M52 108 q40 26 22 66" />
      <path d="M188 88 q-14 40 12 64" />
      <path d="M84 236 q34 -20 76 -6" />
    </g>
    <g fill="#ffb347" opacity=".85">
      <path d="M146 196 l6 -12 l6 12 z" />
      <circle cx="152" cy="198" r="3" />
    </g>
    <g stroke="#6fa8dc" strokeOpacity=".35" strokeDasharray="3 4">
      <path d="M208 91 L256 52" />
      <path d="M206 150 L256 104" />
      <path d="M158 194 L256 156" />
      <path d="M168 208 L256 208" />
      <path d="M134 172 L256 258" />
      <path d="M120 24 L256 302" />
    </g>
    ...
    <text x="262" y="330" fill="#5f7388" fontSize="9">
      各层厚度为示意放大 · 数据：伽利略号 / 哈勃
    </text>
    <text x="24" y="292" fill="#8b97ad" fontSize="9.5">
      木星潮汐力 ↑↓
    </text>
  </svg>
);
...
export default function CosmosViewer() {
  ...
  const fmtRate = (d: number) => {
    const a = Math.abs(d);
    return (
      (d < 0 ? "⇄ " : "×") + (a >= 1 ? a.toFixed(1) : a.toFixed(3)) + " 天 / 秒"
    );
  };
  ...
  return (
    <div
      className="fixed inset-0 overflow-hidden bg-[#03050a] text-[#dce3f0]"
      style={{
        fontFamily:
    ...
    >
      ...
      {webglError && (
        <div className="absolute inset-0 z-[20] flex flex-col items-center justify-center gap-3 text-center px-6">
          <div className="text-[15px] text-[#f5a623] tracking-[.2em]">
            无法启动 3D 渲染
          </div>
          <div className="text-[12px] text-[#9aa7bd] leading-relaxed max-w-[420px]">
            当前浏览器或设备未启用 WebGL。请更新浏览器、开启硬件加速，或在支持
            WebGL 的环境 （如桌面版 Chrome / Edge / Firefox）中打开本页面。
          </div>
        </div>
      )}
      ...
      {state.tourActive && tourCaption && (
        <div
          className="pointer-events-none absolute bottom-[110px] left-1/2 -translate-x-1/2 z-[6]
        ...
        >
          <div className="flex items-center justify-center gap-2 mb-0.5">
            ...
            <span className="text-[9px] tracking-[.3em] text-[#f5a623]/80 uppercase">
              Auto Tour · 自动巡航
            </span>
          </div>
          <div className="text-[13px] text-[#f5e6c8] tracking-[.04em] leading-relaxed">
            {tourCaption}
          </div>
          <div className="mt-1 text-[9px] text-[#8b97ad] tracking-[.08em]">
            拖拽取消 · 点击「跳过」进入下一站
          </div>
        </div>
      )}
      ...
      {state.tourActive && (
        <button
          onClick={() => engineRef.current?.tourSkip()}
      ...
      {state.flyMode && (
        <div className="pointer-events-none absolute inset-0 z-[5]">
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10">
            ...
            <div
              className="w-3 h-3 rounded-full border-2 border-[#5fd3ff] absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"
              style={{ boxShadow: "0 0 8px rgba(95,211,255,.8)" }}
            />
          </div>
      ...
      <header
        className="absolute top-3 left-3 z-[6] w-[290px] max-w-[calc(100vw-1.5rem)]
      ...
      >
        ...
        <a
          ...
          className="mt-1 inline-flex items-center gap-1 text-[11px] tracking-[.14em] text-[#5fd3ff] hover:text-[#f5a623] transition-colors"
        >
        ...
        <div className="flex flex-wrap gap-1.5 mt-2.5">
          <Button
            ...
            onClick={() => {
              const p = engineRef.current?.togglePause() ?? false;
              setPaused(p);
            }}
          >
            {paused ? (
          ...
          <Button
            ...
            onClick={() => {
              const nd = -state.dir;
              engineRef.current?.setDir(nd);
              setRevOn(nd < 0);
              engineRef.current?.setRate(spdVal / 1000);
            }}
          >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-7 px-2.5 text-[11px] border border-[rgba(125,165,225,.18)]"
          ...
          <Button
            ...
            onClick={toggleFly}
            title="漫游飞行模式：WASD 移动 · Shift 加速 · 空格/E 上升 · Q 下降 · 拖拽转向"
          >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  onClick={() =>
                    engineRef.current?.setRealScale(!state.realScale)
                  }
                >
                  <Ruler className="w-3 h-3 mr-1" />
                  {state.realScale ? "真实比例" : "真实比例"}
                </Button>
              </TooltipTrigger>
              <TooltipContent>
                真实比例模式：行星距离按真实 AU 线性缩放（地球 1 · 木星 5.2 ·
                海王星 30），
          ...
          <Button
            ...
            onClick={() =>
              engineRef.current?.toggleLayer("lab", !state.show.lab)
            }
            title="显示 / 隐藏天体名称标签"
          >
        ...
        {state.flyMode && (
          <div className="mt-2.5 text-[10px] leading-[1.7] text-[#5fd3ff]/85 bg-[rgba(95,211,255,.06)] border border-[rgba(95,211,255,.25)] rounded-md px-2.5 py-1.5">
            <b className="text-[#5fd3ff]">漫游模式</b> · <b>W/A/S/D</b> 前后左右
            · <b>空格</b>/<b>E</b> 升 · <b>Q</b> 降 · <b>Shift</b> 加速 ·{" "}
            <b>拖拽</b> 转向 · <b>滚轮</b> 调速
            <div className="mt-1.5 flex items-center gap-2">
              <span className="text-[#5fd3ff]/70 tracking-[.1em]">速度</span>
              <Slider
                value={[Math.round(state.flySpeed)]}
                min={1}
                max={200}
                step={1}
              ...
              />
              <b className="text-[#5fd3ff] tabular-nums w-12 text-right">
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-1.5 mt-2.5 text-[11px] text-[#8b97ad]">
          {LAYERS.map((L) => (
            <label
              key={L.key}
              className="flex items-center gap-1 cursor-pointer tracking-[.04em] hover:text-[#dce3f0] transition-colors"
            >
              <Checkbox
                checked={state.show[L.key as keyof typeof state.show]}
                onCheckedChange={(c) =>
                  engineRef.current?.toggleLayer(L.key, !!c)
                }
        ...
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-[#f5a623] text-[11px] tabular-nums">
            {state.eps.toFixed(2)}°
          </span>
        </div>
        <Slider
          ...
          onValueChange={(v) => {
            setEclVal(v[0]);
            engineRef.current?.setObliquity(v[0] / 10);
          }}
          className="[&_[role=slider]]:bg-[#f5a623] [&_[role=slider]]:border-[#f5a623]"
        />
        ...
        {state.horizonMode && (
          <div className="mt-3">
            <div className="text-[10px] tracking-[.22em] text-[#8b97ad] mb-1.5">
              观测站 SITE
            </div>
            <div className="flex gap-2.5">
              <div className="flex-1 min-w-0">
                <Slider
                  ...
                  onValueChange={(v) => {
                    setLatVal(v[0]);
                    engineRef.current?.setLat(v[0] / 10);
                  }}
                ...
                <span className="block text-[10px] text-[#8b97ad] tracking-[.1em] mt-0.5">
                  纬度 {Math.abs(latVal / 10).toFixed(1)}°
                  {latVal < 0 ? "S" : "N"}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <Slider
                  ...
                  onValueChange={(v) => {
                    setLonVal(v[0]);
                    engineRef.current?.setLon(v[0] / 10);
                  }}
                  className="[&_[role=slider]]:bg-[#5fd3ff] [&_[role=slider]]:border-[#5fd3ff]"
                />
                <span className="block text-[10px] text-[#8b97ad] tracking-[.1em] mt-0.5">
                  经度 {Math.abs(lonVal / 10).toFixed(1)}°
                  {lonVal < 0 ? "W" : "E"}
                </span>
              </div>
            </div>
      ...
      <aside
        className={`absolute top-3 right-3 z-[6] w-[262px] max-w-[calc(100vw-1.5rem)]
        rounded-xl border border-[rgba(125,165,225,.18)] bg-[rgba(9,13,22,.62)] backdrop-blur-md p-3.5 shadow-2xl`}
      >
        <div className="flex justify-between items-center gap-2 mb-1.5">
          <span className="text-[10px] tracking-[.32em] uppercase text-[#8b97ad]">
            Body Dossier
          </span>
          <button
            onClick={() => setInfoMin((v) => !v)}
        ...
        {!infoMin && info && (
          <>
            ...
            <div className="text-[10px] tracking-[.28em] text-[#8b97ad] mt-1 mb-2.5 ml-5">
              {info.en}
            </div>
            <div
            ...
            >
              {info.rows.map((r, i) => (
                <div
                  key={i}
                  className="flex justify-between items-baseline py-1.5 gap-2.5
                  border-b border-dashed border-[rgba(125,165,225,.12)] text-[12px]"
                >
                  <span className="text-[#8b97ad] tracking-[.08em] flex-none">
                    {r[0]}
                  </span>
                  <b className="font-medium tabular-nums tracking-[.03em] text-right">
                    {r[1]}
                  </b>
                </div>
              ))}
          ...
          {SCALE_LEVELS.map((L) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[8px] tracking-[.15em] ${active ? "text-[#f5a623]/70" : "text-[#8b97ad]/60"} whitespace-nowrap`}
                >
                  {L.en}
                </span>
              </button>
      ...
      {europaOpen && (
        <aside
          className="absolute bottom-[92px] left-3 z-[6] w-[452px] max-w-[calc(100vw-1.5rem)]
        ...
        >
          ...
          <p className="text-[11px] leading-[1.75] text-[#9fb0c6] mt-1">
            木星的引力让欧罗巴轨道带有微小
            <b className="text-[#f5a623] font-medium">偏心率</b>
            ，冰壳在每一圈公转中被反复揉搓、 拉伸再回弹——
            <b className="text-[#f5a623] font-medium">潮汐加热</b>
            维持了冰壳之下这片液态海。它是太阳系除地球外最有希望孕育生命的地方。
            探测计划：NASA{" "}
            <b className="text-[#f5a623] font-medium">「欧罗巴快船」</b> · ESA{" "}
            <b className="text-[#f5a623] font-medium">「JUICE」</b>。
          </p>
        </aside>
      )}
