...
import {
  buildSolarNeighborhood, buildMilkyWayGalaxy, buildLocalGroup,
  buildNearbyUniverse, buildSuperclusters, buildObservableUniverse, SCALE_LEVELS,
  type LabelSpec, type BodyData,
} from './cosmos-views';
...
export interface BodyInfo {
  n: string; en: string; c: number; rows: [string, string, string?][]; note: string;
  key: string;
}

type LabelEntry = {
  el: HTMLDivElement; obj: THREE.Object3D; up: number; min?: number; grp: string;
  _on?: boolean; _x?: number; _y?: number;
};
...
export class CosmosEngine {
  ...
  private flyYaw = 0; private flyPitch = 0;       // yaw/pitch in radians (overrides cam.theta/phi)
  // Smooth "fly-to" tween when a body is clicked (cancels if user drags)
  private flyTo: { active: boolean; t: number; dur: number;
    fromTheta: number; toTheta: number;
    fromPhi: number; toPhi: number;
    fromDist: number; toDist: number;
  } = { active: false, t: 0, dur: 1.1, fromTheta: 0, toTheta: 0, fromPhi: 0, toPhi: 0, fromDist: 0, toDist: 0 };
  // Auto-tour: flies through a scripted sequence of waypoints across scale levels
  private tour: { active: boolean; idx: number; t: number; } = { active: false, idx: 0, t: 0 };
  private tourWaypoints: { scaleLevel: number; bodyKey?: string; dist: number; dur: number; caption: string }[] = [];
  private show = {
  ...
  private cam = { theta: 0.7, phi: 1.05, dist: 120, wantDist: 120,
    target: new THREE.Vector3(), want: new THREE.Vector3(), focus: null as THREE.Object3D | null };
  ...
  private buildSolarSystem() {
    ...
    for (const b of SYS) {
      ...
      const isGas = b.tex.type === 'giant' || b.tex.type === 'ice' || b.tex.type === 'cloud';
      const mat = makePlanetMaterial(makeTex(b.tex), {
        night: isEarth ? earthNightTex() : undefined,
        nightBoost: isEarth ? 1.0 : 0,
        twilight: isGas ? [255, 180, 110] : isEarth ? [255, 120, 60] : [220, 110, 70],
      ...
      const haloColor = b.n === '地球' ? 0x5fd3ff
        : b.n === '金星' ? 0xf5e6a0
        : b.n === '火星' ? 0xffa070
        : b.n === '木星' ? 0xf5c98a
        : b.n === '土星' ? 0xf0d9a0
      ...
      if (haloColor) {
        const halo = new THREE.Sprite(new THREE.SpriteMaterial({
          map: glowTex([haloColor >> 16 & 255, haloColor >> 8 & 255, haloColor & 255]),
          color: haloColor, blending: THREE.AdditiveBlending,
        ...
        const atmoRGB = b.n === '地球' ? [95, 160, 255] as [number, number, number]
          : b.n === '金星' ? [255, 220, 130] as [number, number, number]
          : b.n === '火星' ? [255, 140, 90] as [number, number, number]
          : b.n === '木星' ? [230, 190, 130] as [number, number, number]
          : b.n === '土星' ? [220, 200, 150] as [number, number, number]
          : b.n === '天王星' ? [150, 220, 230] as [number, number, number]
      ...
      if (b.ring) {
        ...
        const ringMat = isThin
          ? new THREE.MeshBasicMaterial({
              map: thinRingTex(
                b.n === '天王星' ? [155, 220, 222] : [95, 127, 224], 0.5),
              transparent: true, side: THREE.DoubleSide, depthWrite: false, opacity: 0.55 })
          : new THREE.MeshBasicMaterial({
        ...
        if (b.n === '土星') {
          const rInner = innerR, rOuter = outerR;
    ...
    const moon = new THREE.Mesh(this.sphereGeo,
      new THREE.MeshStandardMaterial({ color: 0x9a958c, roughness: 1 }));
    moon.scale.setScalar(0.13);
    moon.userData.body = {
      n: '月球', en: 'THE MOON (LUNA)', sym: '☽', key: '月球', kind: 'moon', rDisp: 0.13, c: 0x9a958c,
      rows: [['半径', '1,737 km'], ['绕地周期', '27.32 天'], ['平均距离', '384,400 km'],
             ['自转', '潮汐锁定'], ['当前日心距', '—', 'dist']] as [string, string, string?][],
      note: '地球唯一的天然卫星，同步自转使其永远以同一面朝向地球；进入地影即发生月食。',
    };
    this.pickables.push(moon);
    earthNode.og.add(moon); earthNode.moon = moon;
    const moonLabel = document.createElement('div');
    moonLabel.className = 'tag moon'; moonLabel.innerHTML = `<b>·</b>月球`;
    this.labelHost.appendChild(moonLabel);
    this.labelEls.push({ el: moonLabel, obj: moon, up: 0.5, min: 30, grp: 'sys' });
    ...
    for (const mo of MOONS) {
      ...
      mesh.userData.body = {
        ...
        rows: [['半径', Math.round(mo.R).toLocaleString('zh-CN') + ' km'],
               ['绕行周期', fmtP(mo.P)], ['轨道半径', mo.aKm.toLocaleString('zh-CN') + ' km'],
               ['发现', mo.by || '—'], ['当前日心距', '—', 'dist']] as [string, string, string?][],
        note: mo.note,
      };
      const ball = new THREE.Mesh(new THREE.SphereGeometry(0.5, 10, 8),
      ...
      this.labelHost.appendChild(el);
      this.labelEls.push({ el, obj: mesh, up: rDisp * 2 + 0.12, min: 30, grp: 'sys' });
    }
  ...
  private makeBelt(N: number, aMin: number, aMax: number, iMax: number,
    sMin: number, sMax: number, color: number, seed: number): THREE.InstancedMesh {
    const m = new THREE.InstancedMesh(new THREE.IcosahedronGeometry(1, 0),
      new THREE.MeshStandardMaterial({ color, roughness: 1 }), N);
    m.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
    const rnd = ((s: number) => () => (s = s * 48271 % 2147483647) / 2147483647)(seed);
    const list: any[] = [];
    for (let k = 0; k < N; k++) {
      const a = aMin + rnd() * (aMax - aMin);
      const o: any = { a, e: rnd() * 0.14, i: (rnd() - 0.5) * iMax, Om: rnd() * 360, w: rnd() * 360, M0: rnd() * 360,
        P: Math.pow(a, 1.5) * 365.25, s: sMin + rnd() * (sMax - sMin),
  ...
  private buildSky() {
    // Align skyGroup to galactic plane-ish basis
    {
      ...
      this.skyGroup.quaternion.setFromRotationMatrix(new THREE.Matrix4().makeBasis(vX, vY, vZ));
    }
    ...
    {
      const N = 7200, pos = new Float32Array(N * 3), col = new Float32Array(N * 3);
      const pal = [[1,1,1],[.78,.86,1],[1,.92,.78],[1,.78,.62]];
      for (let k = 0; k < N; k++) {
        const th = Math.random() * Math.PI * 2;
        const ph = Math.random() < 0.55
          ? Math.PI / 2 + (Math.random() + Math.random() + Math.random() - 1.5) * 0.38
        ...
        const c = pal[(Math.random() * pal.length) | 0], b = 0.35 + Math.random() * 0.65;
        col[k*3] = c[0]*b; col[k*3+1] = c[1]*b; col[k*3+2] = c[2]*b;
      }
      const geo = new THREE.BufferGeometry();
      geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
      geo.setAttribute('color', new THREE.BufferAttribute(col, 3));
      this.skyGroup.add(new THREE.Points(geo, new THREE.PointsMaterial({
        size: 1.5, sizeAttenuation: false, vertexColors: true, transparent: true, opacity: 0.9, depthWrite: false })));
    }
    ...
    for (const [, ra, dec, mag, sp] of STARS) {
      ...
      s.renderOrder = -15; this.starGroup.add(s);
    }
    ...
    DEEP.forEach((d, idx) => {
      const sp = new THREE.Sprite(new THREE.SpriteMaterial({
        map: nebulaTex(d.kind, d.c1, d.c2), blending: THREE.AdditiveBlending,
        depthWrite: false, transparent: true, opacity: 0.92,
        rotation: d.kind === 'galaxy' ? 0.7 : (idx % 5) * 0.4 }));
      sp.position.copy(unitDir(d.ra, d.dec)).multiplyScalar(1950 + (idx % 5) * 80);
      sp.scale.setScalar(1950 * d.size * D2R * 1.35);
      sp.renderOrder = -10;
      sp.userData.body = {
        n: d.n, en: d.id, key: d.id, kind: 'deep', isDeep: true,
        c: (d.c1[0] << 16) | (d.c1[1] << 8) | d.c1[2],
        rows: [['类型', d.type], ['距离', d.dist], ['视星等', d.mag],
               ['视直径', (d.size * 60 | 0) + '′'],
               ['坐标 J2000', fmtRA(d.ra) + '  ' + fmtDec(d.dec)]] as [string, string, string?][],
        note: d.note,
      };
      this.dsoGroup.add(sp); this.pickables.push(sp);
      const el = document.createElement('div');
      el.className = 'tag deep'; el.innerHTML = `<b>✦</b>${d.n}`;
      this.labelHost.appendChild(el);
    ...
    {
      const gp: number[] = [], gc: number[] = [], seg: THREE.Vector3[] = [], R = 2580;
      for (const c of CONS) {
        ...
        for (const [x, y] of c.lines) {
          seg.push(new THREE.Vector3(gp[(base + x) * 3], gp[(base + x) * 3 + 1], gp[(base + x) * 3 + 2]),
                   new THREE.Vector3(gp[(base + y) * 3], gp[(base + y) * 3 + 1], gp[(base + y) * 3 + 2]));
        }
        ...
        anchor.position.set(cx/len, cy/len, cz/len).normalize().multiplyScalar(2720);
        this.scene.add(anchor);
        const el = document.createElement('div');
        el.className = 'tag con'; el.textContent = c.name;
        this.labelHost.appendChild(el);
        this.labelEls.push({ el, obj: anchor, up: 0, min: 34, grp: 'con' });
      }
      const pg = new THREE.BufferGeometry();
      pg.setAttribute('position', new THREE.BufferAttribute(new Float32Array(gp), 3));
      pg.setAttribute('color', new THREE.BufferAttribute(new Float32Array(gc), 3));
      const conGroup = new THREE.Group();
      conGroup.add(new THREE.Points(pg, new THREE.PointsMaterial({
        size: 2.2, sizeAttenuation: false, vertexColors: true, transparent: true, opacity: 0.95, depthWrite: false })));
      conGroup.add(new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(seg),
        new THREE.LineBasicMaterial({ color: 0x41628f, transparent: true, opacity: 0.55, depthTest: false })));
    ...
    const circleLine = (r: number, y: number, plane: string, color: number, op: number, segs = 144) => {
      const pts: THREE.Vector3[] = [];
      for (let k = 0; k <= segs; k++) {
        const a = (k / segs) * Math.PI * 2;
        const c = Math.cos(a) * r, s = Math.sin(a) * r;
        pts.push(plane === 'xy' ? new THREE.Vector3(c, s, 0)
              : plane === 'yz' ? new THREE.Vector3(0, s, c)
    ...
    };
    {
      ...
      this.eclRingGrp = new THREE.Group(); this.skyRoot.add(this.eclRingGrp);
      ...
      for (const [grp, pos, txt] of [
        ...
        [this.eclRingGrp, new THREE.Vector3(-R * 0.87, 0, R * 0.5), '黄道']] as const) {
        const a = new THREE.Object3D(); a.position.copy(pos); (grp as THREE.Group).add(a);
        const el = document.createElement('div');
        el.className = 'tag con'; el.textContent = txt;
        this.labelHost.appendChild(el);
    ...
    const addSkyMark = (size: number, body: any, srcObj: THREE.Object3D, isSun: boolean, isMoon: boolean) => {
      const sp = new THREE.Sprite(new THREE.SpriteMaterial({
        map: softTex([body.c >> 16 & 255, body.c >> 8 & 255, body.c & 255]),
        color: body.c, blending: THREE.AdditiveBlending, depthWrite: false, transparent: true, opacity: 0.95 }));
      sp.scale.setScalar(size); sp.renderOrder = -6;
      const bd = { ...body, isSky: true, kind: 'sky' };
      sp.userData.body = bd;
      this.skyMarks.add(sp); this.pickables.push(sp);
      this.marks.push({ sp, body: bd, srcObj, isSun, isMoon });
      const el = document.createElement('div');
      el.className = 'tag sky'; el.innerHTML = `<b>${body.sym || '·'}</b>${body.n}`;
      this.labelHost.appendChild(el);
      this.labelEls.push({ el, obj: sp, up: size * 0.55, grp: 'sky' });
    };
    addSkyMark(120, SUN, this.sunMesh, true, false);
    addSkyMark(84, this.nodes['地球'].moon.userData.body, this.nodes['地球'].moon, false, true);
  ...
  private buildHorizon() {
    ...
    this.horizonUI.add(circleLine(HZ * Math.cos(30 * D2R), HZ * Math.sin(30 * D2R), 'xz', 0x3f6f9f, 0.2));
    this.horizonUI.add(circleLine(HZ * Math.cos(60 * D2R), HZ * Math.sin(60 * D2R), 'xz', 0x3f6f9f, 0.2));
    ...
    for (const [pos, txt] of [
      [[ HZ, 0, 0], '北 N'], [[-HZ, 0, 0], '南 S'], [[0, 0,  HZ], '东 E'],
      [[0, 0, -HZ], '西 W'], [[0, HZ, 0], '天顶 Z']] as const) {
      const a = new THREE.Object3D(); a.position.set(pos[0], pos[1], pos[2]); this.horizonUI.add(a);
      const el = document.createElement('div');
      el.className = 'tag card'; el.textContent = txt;
      this.labelHost.appendChild(el);
  ...
  private updateHorizonFrame(): number {
    const lstH = (((280.46061837 + 360.98564736629 * this.simT + this.site.lon) / 15) + 24) % 24;
  ...
  private buildComets() {
    ...
    for (const c of COMETS) {
      ...
      const core = new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 8), new THREE.MeshBasicMaterial({ color: 0xffffff }));
      const ball = new THREE.Mesh(new THREE.SphereGeometry(0.85, 10, 8),
        new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false }));
      ball.userData.body = {
        n: c.n, en: c.en, key: c.key, kind: 'comet', rDisp: 0.3, c: c.c,
        rows: [['彗核直径', c.core], ['公转周期', fmtP(c.P!)], ['轨道半长径', c.a.toFixed(2) + ' AU'],
               ['轨道偏心率', c.e.toFixed(4)], ['轨道倾角', c.i.toFixed(1) + '°'],
               ['当前日心距', '—', 'dist']] as [string, string, string?][],
        note: c.note,
      };
      g.add(core, ball); this.pickables.push(ball);
      const mk = (tex: THREE.Texture, n: number) => {
        ...
        for (let k = 0; k < n; k++) {
          ...
          s.frustumCulled = false; g.add(s); arr.push(s);
        }
        return arr;
      };
      this.cometGroup.add(g);
      const el = document.createElement('div');
      el.className = 'tag'; el.innerHTML = `<b>彗</b>${c.n}`;
      this.labelHost.appendChild(el);
      this.labelEls.push({ el, obj: g, up: 1.2, grp: 'sys' });
      this.comets.push({ c, g, core, coma: mk(comaTex, 1), dust: mk(dustTex, 12), ion: mk(ionTex, 10) });
    }
  }
  ...
  private buildZodiAndShadow() {
    ...
    const zt = zodiTex();
    for (const [s, op] of [[560, 0.34], [1000, 0.13]] as const) {
      const p = new THREE.Mesh(new THREE.PlaneGeometry(1, 1),
        new THREE.MeshBasicMaterial({ map: zt, transparent: true, blending: THREE.AdditiveBlending,
          depthWrite: false, side: THREE.DoubleSide, opacity: op }));
      p.rotation.x = -Math.PI / 2;
      p.scale.set(s, s, 1);
      p.renderOrder = -5;
      this.zodiGroup.add(p);
    }
    this.geg = new THREE.Sprite(new THREE.SpriteMaterial({
      map: glowTex([235, 235, 255]), blending: THREE.AdditiveBlending, depthWrite: false, transparent: true, opacity: 0.06 }));
    this.geg.scale.setScalar(300); this.sysGroup.add(this.geg);

    // Earth shadow cones
    const UMBRA_LEN = 5.0, UMBRA_R0 = 0.42, UMBRA_R1 = 0.03, PENUMBRA_LEN = 7.5, PENUMBRA_R1 = 1.7;
    const shadowMat = (r0: number, r1: number, len: number, color: number, a: number) =>
      new THREE.ShaderMaterial({
        uniforms: { uR0: { value: r0 }, uR1: { value: r1 }, uLen: { value: len },
                    uC: { value: new THREE.Color(color) }, uA: { value: a } },
    ...
    const cone = (r0: number, r1: number, len: number, color: number, a: number) => {
    ...
    (this as any)._UMBRA = { LEN: UMBRA_LEN, R0: UMBRA_R0, R1: UMBRA_R1, PLEN: PENUMBRA_LEN, PR1: PENUMBRA_R1 };
  }
  ...
  private buildMeteors() {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      const line = new THREE.Line(geo, new THREE.LineBasicMaterial({
        color: 0xdff0ff, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false }));
      line.frustumCulled = false;
      this.meteorGroup.add(line);
      this.meteors.push({ line, t: 1e9, dur: 1, p: new THREE.Vector3(), d: new THREE.Vector3(), speed: 0, len: 0 });
    }
  }
  private spawnMeteor(m: any) {
    const th = Math.random() * Math.PI * 2, ph = Math.acos(Math.random() * 2 - 1);
    m.p.set(Math.sin(ph) * Math.cos(th), Math.cos(ph), Math.sin(ph) * Math.sin(th)).multiplyScalar(1450);
    m.d.set(Math.random() - 0.5, -(0.25 + Math.random() * 0.75), Math.random() - 0.5).normalize();
    m.t = 0; m.dur = 0.55 + Math.random() * 0.55;
    m.speed = 1500 + Math.random() * 1400; m.len = 70 + Math.random() * 110;
  }
  ...
  private bindEvents() {
    ...
    this.on(dom, 'pointermove', (e) => {
      ...
      if (this.tour.active) { this.tour.active = false; this.onTourCaption?.(''); }
      if (this.flyMode) {
        // In fly mode, drag = look around (mouselook)
        this.flyYaw -= dx * 0.0035;
        this.flyPitch = Math.max(-1.5, Math.min(1.5, this.flyPitch - dy * 0.0035));
      } else if (this.drag.pan) {
        this.cam.focus = null;
        const s = this.cam.dist * 0.0016;
        this.cam.want.addScaledVector(new THREE.Vector3().setFromMatrixColumn(this.camera.matrix, 0), -dx * s)
                .addScaledVector(new THREE.Vector3().setFromMatrixColumn(this.camera.matrix, 1), dy * s);
      } else {
    ...
    this.on(dom, 'wheel', (e) => {
      ...
      if (this.flyMode) {
        ...
        this.flySpeed = Math.max(1, Math.min(200, cur * Math.pow(1.0018, e.deltaY)));
      } else {
        this.cam.wantDist = Math.max(0.8, Math.min(12000, this.cam.wantDist * Math.pow(1.0016, e.deltaY)));
      }
    }, { passive: false });
  ...
  private aimAt(v: THREE.Vector3) {
    ...
    this.cam.phi = Math.max(0.08, Math.min(Math.PI - 0.08, Math.acos(THREE.MathUtils.clamp(d.y, -1, 1))));
  }
  ...
  private startFlyTo(toTheta: number, toPhi: number, toDist: number) {
    ...
    while (dt < -Math.PI) dt += Math.PI * 2;
    this.flyTo = {
      active: true, t: 0, dur: 1.1,
      fromTheta: this.cam.theta, toTheta: this.cam.theta + dt,
      fromPhi: this.cam.phi, toPhi: toPhi,
      fromDist: this.cam.dist, toDist: toDist,
    };
  }
  ...
  private pick(e: PointerEvent | MouseEvent) {
    ...
    if (b.isSky) {
      const rows = [...(b.rows || []), ['地平坐标', '—', 'altaz']] as [string, string, string?][];
    ...
    if (b.isDeep) {
      this.showInfo(b);
      if (this.horizonMode) { this.cam.focus = null; this.aimAt(hit.point); return; }
      this.cam.focus = null;
      const d = this.cam.target.clone().sub(hit.object.position).normalize();
      const toTheta = Math.atan2(d.x, d.z);
      const toPhi = Math.max(0.08, Math.min(Math.PI - 0.08, Math.acos(THREE.MathUtils.clamp(d.y, -1, 1))));
  ...
  private showInfo(b: any) {
    ...
    const info: BodyInfo = {
      n: b.n, en: b.en || '', c: b.c ?? 0xf5a623, key: b.key,
  ...
  private frame = (now: number) => {
    ...
    for (const key in this.nodes) {
      ...
      if (pmat) {
        ...
        (pmat.uniforms.uSunDir.value as THREE.Vector3).copy(this._ew).negate().normalize();
      }
      // Saturn ring shadow: same sun direction
      const rsm = nd.mesh.userData.ringShadowMat;
      if (rsm) {
        nd.mesh.getWorldPosition(this._ew);
        (rsm.uniforms.uSunDir.value as THREE.Vector3).copy(this._ew).negate().normalize();
      }
      // Atmosphere rim (Fresnel): same sun direction
      const atm = nd.mesh.userData.atmoMat;
      if (atm) {
        nd.mesh.getWorldPosition(this._ew);
        (atm.uniforms.uSunDir.value as THREE.Vector3).copy(this._ew).negate().normalize();
      }
      if (nd.moon) {
        const a = (2 * Math.PI * this.simT) / 27.322 + 0.8;
        nd.moon.position.set(Math.cos(a) * 1.4, Math.sin(a) * 0.18, Math.sin(a) * 1.4);
      }
    }
    this.liveDist.sun = '—';
    for (const mo of MOONS) {
    ...
    if (this.cometGroup.visible) {
      for (const cm of this.comets) {
        ...
        for (let k = 0; k < cm.dust.length; k++) {
          const s = (k + 1) / cm.dust.length, sp = cm.dust[k];
          sp.position.copy(this._d).multiplyScalar(L * s * 1.02).addScaledVector(this._q, L * 0.3 * s * s);
        ...
        for (let k = 0; k < cm.ion.length; k++) {
          const s = (k + 1) / cm.ion.length, sp = cm.ion[k];
    ...
    if (this.shadowGrp.visible) {
      ...
      if (s > 0 && s < U.LEN) {
        ...
        moonMat.color.setHex(perp < uR ? 0x8a4034 : (perp < pR ? 0x6d6259 : this.MOON_BASE));
    ...
    if (this.horizonMode) {
      ...
      for (const m of this.marks) {
        ...
        this.liveAltaz[m.body.key] = fmtDeg(alt) + '  高度 · ' + az.toFixed(1) + '° 方位';
      }
      this.cam.want.set(0, 0, 0); this.cam.wantDist = 2;
    } else {
    ...
    if (this.meteorGroup.visible) {
      ...
      for (const m of this.meteors) {
        if (m.t > m.dur) { m.line.material.opacity = 0; continue; }
        m.t += dt;
    ...
    for (let i = 1; i < this.cosmosViews.length; i++) {
      ...
      const exoUpdate = g.userData.exoUpdate as ((t: number) => void) | undefined;
      if (exoUpdate) exoUpdate(this.simT);
      // gentle pulse on the galactic core sprite (first child of Milky Way)
      if (i === 2 && g.children[1] && (g.children[1] as THREE.Sprite).material) {
    ...
    if (this.flyTo.active) {
      ...
      this.cam.theta = this.flyTo.fromTheta + (this.flyTo.toTheta - this.flyTo.fromTheta) * e;
      this.cam.phi = this.flyTo.fromPhi + (this.flyTo.toPhi - this.flyTo.fromPhi) * e;
      this.cam.dist = this.flyTo.fromDist + (this.flyTo.toDist - this.flyTo.fromDist) * e;
    ...
    if (this.flyMode) {
      ...
      const cp = Math.cos(this.flyPitch), sp = Math.sin(this.flyPitch);
      const fwd = this._d.set(Math.sin(this.flyYaw) * cp, sp, Math.cos(this.flyYaw) * cp).normalize();
      ...
      const speed = (this.keys['shift'] ? 6 : 2) * this.flySpeed * (this.scaleLevel === 0 ? 1 : 3);
      const want = this._p.set(0, 0, 0);
      if (this.keys['w'] || this.keys['arrowup']) want.addScaledVector(fwd, 1);
      if (this.keys['s'] || this.keys['arrowdown']) want.addScaledVector(fwd, -1);
      if (this.keys['a'] || this.keys['arrowleft']) want.addScaledVector(right, -1);
      if (this.keys['d'] || this.keys['arrowright']) want.addScaledVector(right, 1);
      ...
      this.camera.position.set(
        this.cam.target.x + this.cam.dist * Math.sin(sp2) * Math.sin(this.cam.theta),
        this.cam.target.y + this.cam.dist * Math.cos(sp2),
        this.cam.target.z + this.cam.dist * Math.sin(sp2) * Math.cos(this.cam.theta));
      const d = this._q.copy(this.cam.target).sub(this.camera.position).normalize();
      if (this.horizonMode) this.camera.up.copy(Math.abs(this._uU.dot(d)) > 0.9 ? this._uN : this._uU);
    ...
    if (this.labelHost.style.display !== 'none') {
      ...
      for (const L of this.labelEls) {
        ...
        const grpOk = !L.grp ? true
          ...
          : L.grp === 'sys' ? (!this.horizonMode && this.scaleLevel === 0)
          : (L.grp === 'sky' || L.grp === 'hzd') ? this.horizonMode
          : L.grp === 'ring' ? this.scaleLevel === 0
  ...
  setDir(dir: number) { this.dir = dir; }
  togglePause() { this.paused = !this.paused; return this.paused; }
  setRealTime() { this.daysPerSec = 1 / 86400; }
  jumpToNow() { this.simT = (Date.now() - EPOCH) / 86400000; }
  resetView() {
    this.cam.focus = null; this.cam.want.set(0, 0, 0);
    this.cam.wantDist = this.horizonMode ? 2 : SCALE_LEVELS[this.scaleLevel].sceneScale;
    this.cam.theta = 0.7; this.cam.phi = 1.05; this.showInfo(SUN);
  }
  toggleHorizon(): boolean {
    ...
    this.cam.focus = null;
    if (this.horizonMode) { this.cam.want.set(0, 0, 0); this.cam.wantDist = 2; this.camera.fov = 62; }
    else this.camera.fov = 52;
    this.camera.updateProjectionMatrix();
    this.showInfo(SUN);
    return this.horizonMode;
  }
  setObliquity(v: number) { this.EPS = v; this.applyEps(); }
  setLat(v: number) { this.site.lat = v; }
  setLon(v: number) { this.site.lon = v; }
  /** Jump simulation time to an absolute day count since J2000 epoch. */
  setSimTime(days: number) { this.simT = days; }
  ...
  toggleFly(): boolean {
    ...
    if (this.flyMode) {
      ...
      const fwd = this._p.copy(this.cam.target).sub(this.camera.position).normalize();
  ...
  setFlySpeed(v: number) { this.flySpeed = Math.max(1, Math.min(200, v)); }
  ...
  toggleTour(): boolean {
    ...
    this.tourWaypoints = [
      { scaleLevel: 0, bodyKey: 'sun',     dist: 14,  dur: 4, caption: '太阳 · G2V 主序星，太阳系的中心' },
      { scaleLevel: 0, bodyKey: '地球',     dist: 4,   dur: 4, caption: '地球 · 唯一已知存在液态水海洋的行星' },
      { scaleLevel: 0, bodyKey: '木星',     dist: 12,  dur: 4, caption: '木星 · 质量为其余七颗行星总和的 2.5 倍' },
      { scaleLevel: 0, bodyKey: '土星',     dist: 10,  dur: 4, caption: '土星 · 环系宽达 28 万公里却厚不足百米' },
      { scaleLevel: 1, dist: 14,            dur: 5, caption: '近邻恒星 · 太阳周围 16 光年内的恒星邻居' },
      { scaleLevel: 2, dist: 35,            dur: 5, caption: '银河系 · 约 4000 亿颗恒星组成的棒旋星系' },
      { scaleLevel: 3, dist: 34,            dur: 5, caption: '本星系群 · 银河系与仙女座星系为两大核心' },
      { scaleLevel: 4, dist: 45,            dur: 5, caption: '近邻宇宙 · 室女座星系团是本超星系团引力中心' },
      { scaleLevel: 5, dist: 95,            dur: 5, caption: '超星系团网络 · 星系沿宇宙网状结构聚集' },
      { scaleLevel: 6, dist: 30,            dur: 6, caption: '可观测宇宙 · 直径约 930 亿光年，最外层是 CMB' },
    ];
  ...
  setTourCaptionCallback(cb: (caption: string) => void) { this.onTourCaption = cb; }
  isTourActive() { return this.tour.active; }
  ...
  private tourTick(dt: number) {
    ...
    if (!wp) { this.tour.active = false; return; }
    ...
    if (this.tour.t === 0) {
      ...
      if (wp.bodyKey && wp.scaleLevel === 0) {
        const nd = Object.values(this.nodes).find((n: any) => n.body.key === wp.bodyKey);
        if (nd) {
  ...
  toggleLayer(key: string, on: boolean) {
    ...
    if (key === 'orb') { this.orbitGroupRoot.visible = on; for (const r of this.subOrbits) r.visible = on; }
    if (key === 'lab') this.labelHost.style.display = on ? '' : 'none';
    if (key === 'belt') { this.beltAst.visible = on; this.beltKbo.visible = on; }
    if (key === 'mw') { const mw = this.skyGroup.getObjectByName('mw'); if (mw) mw.visible = on; }
    if (key === 'dso') { this.dsoGroup.visible = on; this.starGroup.visible = on; this.showGrp.dso = on; }
    if (key === 'con') { this.CONG.visible = on; this.showGrp.con = on; }
    if (key === 'comet') this.cometGroup.visible = on;
    if (key === 'met') this.meteorGroup.visible = on;
    if (key === 'zodi') { this.zodiGroup.visible = on; this.geg.visible = on; }
